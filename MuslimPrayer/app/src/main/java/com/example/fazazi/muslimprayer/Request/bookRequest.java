...
public class bookRequest {
    ...
    public bookRequest(RequestQueue queue) {
        this.queue = queue;
    }

    public void getSongList(final bookInteface callback) {
        String lan = Locale.getDefault().getLanguage();
        String URL = "http://api.islamhouse.com/v1/lxoPK9Obr8vqoLcZ/dynamic/ramadan/get-content-by-type/books/ar/ar/1/50/json";
                ...
                if (jsonArray.length() > 0 && null != jsonArray) {
                    ...
                    for (int i = 0; i < u; i++) {
                        try {
                            ...
                            JSONArray o = songObject.getJSONArray("prepared_by");
                            String prepar = o.getJSONObject(0).getString("title");

                            JSONArray a = songObject.getJSONArray("attachments");
                            String url = a.getJSONObject(0).getString("url");

                            book book = new book(title, url, prepar, description);
